...
from projects.scmb.robothor_plugin.robothor_sensors import DepthSensorThor
from projects.scmb.robothor_plugin.robothor_task_samplers import (
    ObjectNavDatasetTaskSampler,
)
from projects.scmb.robothor_plugin.robothor_tasks import ObjectNavTask
from projects.scmb.objectnav.experiments.objectnav_base import ObjectNavBaseConfig

if ai2thor.__version__ not in ["0.0.1", None] and version.parse(
    ai2thor.__version__
) < version.parse("3.2.0"):
    raise ImportError(
        "To run the AI2-THOR ObjectNav baseline pointnav you must use"
...
class ObjectNavThorBaseConfig(ObjectNavBaseConfig, ABC):
    """The base config for all AI2-THOR ObjectNav pointnav."""
    ...
    TRAIN_ROTATION_BIAS: Optional[List[float]] = None
    VAL_ROTATION_BIAS: Optional[List[float]] = None
    TEST_ROTATION_BIAS: Optional[List[float]] = None
    TRAIN_MOVEMENT_BIAS: Optional[List[float]] = None
    VAL_MOVEMENT_BIAS: Optional[List[float]] = None
    TEST_MOVEMENT_BIAS: Optional[List[float]] = None
    NUM_INTERMEDIATE_VISUAL_OBSERVATION: int = 0

    TEST_NUM_BROKEN_ROTATION_ACTIONS: int = 0
    TEST_CONTEXT_BROKEN: bool = False
    TEST_VISUALIZE_OR_NOT: bool = False
    ...
    def _get_sampler_args_for_scene_split(
        ...
        return {
            ...
            "num_intermediate_visual_observation": self.NUM_INTERMEDIATE_VISUAL_OBSERVATION,
        }
    ...
    def train_task_sampler_args(
        ...
        res["rotation_bias"] = self.TRAIN_ROTATION_BIAS
        res["movement_bias"] = self.TRAIN_MOVEMENT_BIAS
        return res
    ...
    def valid_task_sampler_args(
        ...
        res["rotation_bias"] = self.VAL_ROTATION_BIAS
        res["movement_bias"] = self.VAL_MOVEMENT_BIAS
        return res
    ...
    def test_task_sampler_args(
        ...
        if self.test_on_validation or self.TEST_DATASET_DIR is None:
            ...
            res["scene_directory"] = self.VAL_DATASET_DIR
            res["loop_dataset"] = False
            res["rotation_bias"] = self.TEST_ROTATION_BIAS
            res["movement_bias"] = self.TEST_MOVEMENT_BIAS
            res["num_broken_rotation_actions"] = self.TEST_NUM_BROKEN_ROTATION_ACTIONS
            res["context_broken"] = self.TEST_CONTEXT_BROKEN
            res["visualize"] = self.TEST_VISUALIZE_OR_NOT
            return res
