...
from projects.scmb.robothor_plugin.robothor_tasks import (
    ObjectNavTask,
    PointNavTask,
    NavToPartnerTask,
)
...
class ObjectNavDatasetTaskSampler(TaskSampler):
    def __init__(
        ...
        rotation_bias: Optional[List[float]] = None,
        movement_bias: Optional[List[float]] = None,
        num_broken_rotation_actions: int = 0,
        context_broken: bool = False,
        visualize: bool = False,
        env_class=RoboThorEnvironment,
        randomize_materials_in_training: bool = False,
        num_intermediate_visual_observation: int = 0,
        **kwargs,
    ) -> None:
        self.rotation_bias = rotation_bias
        self.movement_bias = movement_bias
        self.num_broken_rotation_actions = num_broken_rotation_actions
        self.context_broken = context_broken
        self.visualize = visualize
        self.num_intermediate_visual_observation = num_intermediate_visual_observation
        self.rewards_config = rewards_config
        self.env_args = env_args
        self.scenes = scenes
        self.episodes = {
    ...
    def next_task(self, force_advance_scene: bool = False) -> Optional[ObjectNavTask]:
        ...
        if self.rotation_bias is not None:
            rotation_bias = random.uniform(self.rotation_bias[0], self.rotation_bias[1])
            if len(self.rotation_bias) > 2:
                while rotation_bias > self.rotation_bias[2] and rotation_bias < self.rotation_bias[3]:
                    rotation_bias = random.uniform(self.rotation_bias[0], self.rotation_bias[1])
        else:
            rotation_bias = None

        if self.movement_bias is not None:
            movement_bias = random.uniform(self.movement_bias[0], self.movement_bias[1])
            if len(self.movement_bias) > 2:
                while movement_bias > self.movement_bias[2] and movement_bias < self.movement_bias[3]:
                    movement_bias = random.uniform(self.movement_bias[0], self.movement_bias[1])
        else:
            movement_bias = None

        if self.num_broken_rotation_actions > 0:
            if self.context_broken:
                # start_id = np.random.choice(12, 1)[0]
                # start_id = 1 # 2 for right
                start_id = 7 # 8 for left
                broken_rotation_actions_id = list((np.roll(
                    np.arange(12),
                    -start_id
                ) + 3)[:self.num_broken_rotation_actions])
            else:
                broken_rotation_actions_id = list(np.random.choice(
                    12,
                    self.num_broken_rotation_actions,
                    replace=False
                ) + 3)
        else:
            broken_rotation_actions_id = []
        ...
        task_info["rotation_bias"] = rotation_bias
        task_info["movement_bias"] = movement_bias
        task_info["broken_rotation_actions_id"] = broken_rotation_actions_id
        task_info["visualize"] = self.visualize
        task_info["num_intermediate_visual_observation"] = self.num_intermediate_visual_observation
        if self.allow_flipping and random.random() > 0.5:
            task_info["mirrored"] = True
        else:
            task_info["mirrored"] = False
...
class PointNavDatasetTaskSampler(TaskSampler):
    ...
    def next_task(self, force_advance_scene: bool = False) -> Optional[PointNavTask]:
        ...
        task_info = {
            ...
            "broken_rotation_actions_id": broken_rotation_actions_id,
