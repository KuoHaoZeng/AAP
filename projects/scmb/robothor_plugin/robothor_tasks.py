...
from projects.scmb.robothor_plugin.robothor_constants import (
    MOVE_AHEAD,
    ROTATE_LEFT,
    ROTATE_RIGHT,
    END,
...
def soft_spl_metric(
        initial_distance: float, final_distance: float, optimal_distance: float, travelled_distance: float
) -> Optional[float]:
    if optimal_distance < 0:
        ...
        travelled_distance = max(travelled_distance, optimal_distance)
        soft_success = (1 - final_distance / initial_distance)
        return soft_success * optimal_distance / travelled_distance


class PointNavTask(Task[RoboThorEnvironment]):
    _actions = ([(MOVE_AHEAD, 0.25)],
                [(MOVE_AHEAD, 0.15)],
                [(MOVE_AHEAD, 0.05)],
                [(ROTATE_RIGHT, 0)],
                [(ROTATE_RIGHT, 30)],
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Hao: add for the state-change model-base project
        self.initial_agent_state = self.env.agent_state()
        self.cur_state_observation = None
        self.cur_visual_observation = None
        self.cur_depth_observation = None
        self.prev_state_observation = None
        self.prev_visual_observation = None
        self.prev_depth_observation = None
        self.last_action_success = True
        if "rotation_bias" in self.task_info:
            self.rotation_bias = self.task_info["rotation_bias"]
            self.movement_bias = self.task_info["movement_bias"]
        else:
            self.rotation_bias = None
            self.movement_bias = None

        self.last_action_ind = 0

        if "broken_rotation_actions_id" in self.task_info:
            self.broken_rotation_actions_id = self.task_info["broken_rotation_actions_id"]
        else:
            self.broken_rotation_actions_id = []
        self.took_actions = []
        self.took_actions_magnitude = []
        self.took_actions_id = []
        self.img_id = 1
    ...
    def _step(self, action: Union[int, Sequence[int]]) -> RLStepResult:
        if self.task_info["visualize"]:
            d = "smooth_disabled_results_r{}_m{}/{}_{}".format(
                self.rotation_bias,
                self.movement_bias,
                self.task_info["id"],
                len(self.broken_rotation_actions_id),
            )
            if not os.path.isdir(d):
                os.makedirs(d, exist_ok=True)
            Image.fromarray(self.env.last_event.frame).save(
                "{}/egocentric_{:03}.png".format(d, self.img_id))
            self.img_id += 1

        self.prev_visual_observation = self.cur_visual_observation
        self.prev_depth_observation = self.cur_depth_observation
        self.prev_state_observation = self.cur_state_observation

        assert isinstance(action, int)
        action = cast(int, action)

        self.last_action_ind = action
        self.took_actions_id.append(action)

        if action not in self.broken_rotation_actions_id:

            action_str_set = self.class_action_names()[action]

            for (action_str, magnitude) in action_str_set:
                self.took_actions.append(action_str)
                self.took_actions_magnitude.append(magnitude)
                if action_str == END:
                    self._took_end_action = True
                    self._success = self._is_goal_in_range()
                    self.last_action_success = self._success
                else:
                    if action_str in [ROTATE_RIGHT, ROTATE_LEFT]:
                        # Hao: add the rotation bias
                        if self.rotation_bias is not None:
                            magnitude += self.rotation_bias
                        action_dict = {"action": action_str, "degrees": magnitude}
                    elif action_str in [MOVE_AHEAD]:
                        if self.movement_bias is not None:
                            magnitude += self.movement_bias
                        action_dict = {"action": action_str, "moveMagnitude": magnitude}
                    else:
        ...
        if self.task_info["visualize"]:
            if self._took_end_action or self.is_done() or self.num_steps_taken() >= 499:
                d = "smooth_disabled_results_r{}_m{}/{}_{}".format(
                    self.rotation_bias,
                    self.movement_bias,
                    self.task_info["id"],
                ...
                if not os.path.isdir(d):
                    os.makedirs(d, exist_ok=True)
                Image.fromarray(self.env.last_event.frame).save("{}/egocentric_{:03}.png".format(d, self.img_id))
                spl = spl_metric(
                ...
                log = {
                    "took_actions": self.took_actions,
                    "took_actions_magnitude": self.took_actions_magnitude,
                    "success": self._success,
                    "spl": 0 if spl is None else spl,
                    "total_reward": float(np.sum(self._rewards)),
                    "dist_to_target": self.dist_to_target(),
                    "length": len(self.took_actions),
                    "rotation_bias": self.rotation_bias,
                    "movement_bias": self.movement_bias,
                    "broken_actions": [self.class_action_names()[ele] for ele in self.broken_rotation_actions_id],
                    "num_broken_actions": len(self.broken_rotation_actions_id)
                }
                json.dump(log, open("{}/log.json".format(d), "w"), indent=6)
        return step_result
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        soft_spl = soft_spl_metric(
            initial_distance=self.optimal_distance,
            final_distance=self.dist_to_target(),
            optimal_distance=self.optimal_distance,
            travelled_distance=self.travelled_distance,
        )

        self.task_info["scmb_log"] = {
            "took_actions": self.took_actions,
            "took_actions_id": self.took_actions_id,
            "took_actions_magnitude": self.took_actions_magnitude,
            "success": self._success,
            "spl": 0 if spl is None else spl,
            "soft_spl": 0 if soft_spl is None else soft_spl,
            "total_reward": float(np.sum(self._rewards)),
            "dist_to_target": self.dist_to_target(),
            "length": len(self.took_actions),
            "rotation_bias": self.rotation_bias,
...
class ObjectNavTask(Task[RoboThorEnvironment]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "rotation_bias" in self.task_info:
            ...
            self.rotation_bias = None
            self.movement_bias = None
        self.num_intermediate_visual_observation = self.task_info["num_intermediate_visual_observation"]
        self.intermediate_visual_observation = [None] * self.num_intermediate_visual_observation
    ...
    def _step(self, action: Union[int, Sequence[int]]) -> RLStepResult:
        if self.task_info["visualize"]:
            d = "objectnav_disabled_results_r{}_m{}/{}_{}".format(
                self.rotation_bias,
                self.movement_bias,
                self.task_info["id"],
                len(self.broken_rotation_actions_id),
            ...
            if not os.path.isdir(d):
                os.makedirs(d, exist_ok=True)
            Image.fromarray(self.env.last_event.frame).save("{}/egocentric_{:03}.png".format(d, self.num_steps_taken() + 1))
        ...
        if action not in self.broken_rotation_actions_id:
            ...
            for (action_str, magnitude) in action_str_set:
                ...
                if action_str == END:
                    ...
                    sub_magnitude = magnitude / (self.num_intermediate_visual_observation + 1)
                    if self.rotation_bias is not None:
                        sub_rotation_bias = self.rotation_bias / (self.num_intermediate_visual_observation + 1)
                    else:
                        sub_rotation_bias = 0
                    if self.movement_bias is not None:
                        sub_movement_bias = self.movement_bias / (self.num_intermediate_visual_observation + 1)
                    else:
                        sub_movement_bias = 0
                    for sub_step in range(self.num_intermediate_visual_observation + 1):
                        if action_str in [ROTATE_RIGHT, ROTATE_LEFT]:
                            # Hao: add the rotation bias
                            action_dict = {"action": action_str, "degrees": sub_magnitude + sub_rotation_bias}
                        elif action_str in [MOVE_AHEAD]:
                            action_dict = {"action": action_str, "moveMagnitude": sub_magnitude + sub_movement_bias}
                        else:
                            raise NotImplementedError
                        self.env.step(action_dict)
                        self.last_action_success = self.env.last_action_success
                        ...
                        self.path.append({k: pose[k] for k in ["x", "y", "z"]})
                        # self.task_info["followed_path"].append(pose)
                        if sub_step < len(self.intermediate_visual_observation):
                            self.intermediate_visual_observation[sub_step] = self.env.current_frame.copy()
                if len(self.path) > 1:
                    self.travelled_distance += IThorEnvironment.position_dist(
                        p0=self.path[-1], p1=self.path[-2], ignore_y=True
                    )
        ...
        if self.task_info["visualize"]:
            if self._took_end_action or self.is_done() or self.num_steps_taken() >= 499:
                ...
                if not os.path.isdir(d):
                    os.makedirs(d, exist_ok=True)
                Image.fromarray(self.env.last_event.frame).save("{}/egocentric_{:03}.png".format(d, self.num_steps_taken() + 2))
                spl = spl_metric(
                    success=self._success,
                    optimal_distance=self.optimal_distance,
                    travelled_distance=self.travelled_distance,
