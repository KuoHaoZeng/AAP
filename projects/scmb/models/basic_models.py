...
class OrderInvariantActorCritic(ActorCriticModel[CategoricalDistr]):
    def __init__(
            self,
            input_uuid: str,
            action_space: gym.spaces.Discrete,
    ...
    def forward(self, observations, memory, prev_actions, masks):
        out = self.linear(observations[self.input_uuid]).squeeze(-1)
...
class OrderInvariantSoftAttentionActorCritic(ActorCriticModel[CategoricalDistr]):
    def __init__(
            self,
            input_uuid: str,
            action_space: gym.spaces.Discrete,
    ...
    def forward(self, observations, memory, prev_actions, masks):
        nsteps, nsamplers, n_action_value = observations[self.input_uuid].shape[:3]
        action_embed = observations[self.input_uuid][:, :, -1, :]
        v_out = self.linear(action_embed)

        action_embed = action_embed.reshape(nsteps * nsamplers, -1).unsqueeze(-1)
        rep = observations[self.input_uuid][:, :, :-1].reshape(nsteps * nsamplers, n_action_value - 1, -1)
        out = torch.bmm(rep, action_embed).reshape(nsteps, nsamplers, n_action_value - 1)

        # noinspection PyArgumentList
        return (
            ActorCriticOutput(
                # ensure [steps, samplers, ...]
                distributions=CategoricalDistr(logits=out),
                # ensure [steps, samplers, flattened]
                values=cast(torch.FloatTensor, v_out),
                extras={},
            ),
            None,
        )
