...
class TransformerHead(nn.Sequential):
    def __init__(self, nfeatures, ninp, nhead, nhid, dropout, initrange, ndecoder):
        layers = []
        for _ in range(ndecoder):
            layers.append(TransformerDecoderLayer(ninp, nhead, nhid, dropout))

        layers.append(LinearLayer(ninp, nfeatures, initrange))
        super(TransformerHead, self).__init__(*layers)


class TransformerHeadEN(nn.Sequential):
    def __init__(self, nfeatures, ninp, nhead, nhid, dropout, initrange, ndecoder):
        layers = []
        for _ in range(ndecoder):
            layers.append(TransformerEncoderLayer(ninp, nhead, nhid, dropout))

        layers.append(LinearLayer(ninp, nfeatures, initrange))
        super(TransformerHeadEN, self).__init__(*layers)
